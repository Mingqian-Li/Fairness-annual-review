...
# display performance (ROC AUC for landmark 4 years old a and for different combination of variables)
#source(paste0(codePath,"figure1_4Years Landmark.R"))

fig1 <- ggdraw() +
  draw_plot(p, x = 0, y = 0, width = 0.5, height = 1) +
  #draw_plot(p_4, x = 0.5, y = 0, width = 0.5, height = 1) +
  draw_plot_label(label = c("a", "b"), size = 15,
                  x = c(0, 0.5), y = c(1,1))
fig1
ggsave(paste0(pathfigures_tables,"figure1_new.jpg"),width = 6.8*1.7, height = 6.8*1.7/2)


#read_pptx() %>%
#  add_slide(layout = "Title and Content", master = "Office Theme") %>%
#  ph_with_vg(code = print(fig1), type = "body") %>%
#  print(target = paste0(pathfigures_tables,"figure1_new.pptx"))
