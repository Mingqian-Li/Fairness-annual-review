tAUC_ml_v <- function(complexity1,dataset_ml,dataset_timedep,day_begin,day_end,weighting = "marginal", variable_of_interest = "t1d", path = pathSaveModels,control = trainControl(method = "repeatedcv", number = 5, repeats = 2,classProbs = TRUE,summaryFunction = twoClassSummary,savePredictions = TRUE)){
  ...
  variables_for_prediction <- list_variable_f(day_begin,complexity1)
...
TimeRoc1 <- tAUC_ml_v(complexity1,dataset_ml,dataset_ml,day,day + day_endv, variable_of_interest = "t1d")


TP <- TimeRoc1$TP
colnames(TP) <- c("year1","years3","years5","years8")
FP <- TimeRoc1$FP
colnames(FP) <- c("year1","years3","years5","years8")


TP <- TP %>% data.frame() %>% gather("year","TP",1:length(day_endv)) %>%  mutate(year = factor(year, levels = c("year1","years3","years5","years8","years10")))
...
source(paste0(codePath,"Extract_information_per_date.R"))
dataset_ml <- finaldata
TimeRoc2 <- tAUC_ml_v(complexity1,dataset_ml,dataset_ml,day,day + day_endv, variable_of_interest = "t1d")  
...
p1 <- ggplot(data_all, aes(x = FP,y = TP, colour = year)) +
  geom_line(aes(linetype = model),size = 1) +
  coord_equal() +
  style_roc(ylab = "sensitivity", xlab = "1 - specificity") +
  scale_linetype_manual(name = "Model", values = c("solid", "twodash"), labels = c("simple fdr","weighted fdr")) +
  scale_colour_brewer(palette = "Set1",name = "Horizon time", labels = c("1 year","3 years","5 years","8 years")) +
  theme(legend.position = c(0.8,0.4),text = element_text(size = 14,face = "bold"))
p1

ggsave(paste0(pathfigures_tables,"figureS1B.jpg"),width = 6.8,height = 6.8)
# 
# library("tidyverse")
# library("officer")
# library("rvg")
...
#   print(target = "C:/Users/Lauric/Dropbox/TEDDY prediction model paper/figures and tables/figureS3PanelB.pptx")

compare(TimeRoc1, TimeRoc2, adjusted = TRUE, abseps = 1e-06)
