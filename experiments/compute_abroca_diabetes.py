...
def compute_abroca_diabetes(
    ...
    if len(df[label_col].value_counts()) == 2:
        ...
        exit(1)
    if len(df[protected_attr_col].value_counts()) == 5:
        pass
    else:
        print("The protected attribute column should be the same with races numbers")
        exit(1)
    # initialize data structures
    # slice_score = 0
    prot_attr_values = df[protected_attr_col].value_counts().index.values
    fpr_tpr_dict = {}
    minority_protected_attr_val_list=[]
    ...
    for pa_value in prot_attr_values:
        if pa_value != majority_protected_attr_val:
            minority_protected_attr_val_list.append(pa_value)
    ...
    minority_roc_x_list=[]
    minority_roc_y_list=[]
    for i in range(len(minority_protected_attr_val_list)):
        minority_roc_x_, minority_roc_y_ = interpolate_roc_fun(
        fpr_tpr_dict[minority_protected_attr_val[i]][0],
        fpr_tpr_dict[minority_protected_attr_val[i]][1],
        n_grid,)
        minority_roc_x_list.append(minority_roc_x_)
        minority_roc_y_list.append(minority_roc_y_)
    ...
    if plot_slices == True:
        slice_plot(
            ...
            minority_group_name=minority_group_name_1,
            fout=file_name,
            value=round(slice,4),
        )

    return slice, minority_roc_x
