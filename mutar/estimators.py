...
from .solvers import solver_dirty, solver_lasso, solver_mll, solver_mtw,solver_logistic
from . import utils
...
class MultitaskRegression(BaseEstimator, RegressorMixin):
    ...
    def _fit(self, X, y,n_samples_list):
...
class DirtyModel(MultitaskRegression):
    ...
    def _fit(self, X, y,n_samples_list):
        ...
        coef_shared_, coef_specific_, R, n_iter = \
            solver_dirty(X, y, coef_shared_, coef_specific_,n_samples_list,
...
class GroupLasso_Sparse(DirtyModel):
    """ GroupLasso estimator with L1/L2 mixed-norm as regularizer.
...
class IndRewLogisticL1(MultitaskRegression):
    """ Independent Reweighted Lasso estimator with L1 regularizer.
    ...
    def _fit(self, X, y):
        ...
        for i in range(self.max_iter_reweighting):
            Xw = X * weights.T[:, None, :]
            coef_ = solver_logistic(Xw, y, self.alpha, self.max_iter, self.tol,
                                 positive=self.positive)
            coef_ = coef_ * weights
            err = abs(coef_ - coef_old).max()
            err /= max(abs(coef_).max(), abs(coef_old).max(), 1.)
