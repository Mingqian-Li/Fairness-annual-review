...
def solver_dirty(X, y, coef_shared_, coef_specific_,n_samples_list, alpha=1., beta=1.,
    ...
    coef_shared_, coef_specific_, R, n_iter = \
        _solver_dirty(X, R, coef_shared_, coef_specific_,n_samples_list, Ls, alpha, beta,
...
def _solver_dirty(X, R, coef_shared_, coef_specific_,n_samples_list, Ls, alpha, beta,
                  max_iter, tol, positive):
    """ Coordinate solver of DirtyModels in numba."""
    n_tasks = len(X)
    n_samples, n_features = X[0].shape
    theta = coef_shared_ + coef_specific_
    alpha *= sum(n_samples_list)/len(n_samples_list)
    beta *= sum(n_samples_list)/len(n_samples_list)
    ...
    for i in range(max_iter):
        ...
        for j in range(n_features):
            ...
            for t in range(n_tasks):
                for n in range(n_samples_list[t]):
...
def solver_logistic(X, y, alpha=None, max_iter=3000, tol=1e-4, positive=False):
    """Solver for Independent Lasso."""
